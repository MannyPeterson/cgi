...
int main(int argc, char **argv)
{
    ...
    KeyValuePair *p = (KeyValuePair *)calloc(MAX_KEYVALUE_PAIRS, sizeof(KeyValuePair));
    int i = 0;
    int n = 0;

    printf("Content-Type: text/html\n\n");

    if (RETURN_SUCCESS == getKeyValuePairs(p, &n))
    {
        printf("<!DOCTYPE html><html><body><table border=\"1\"><tr><td>KEY</td><td>VALUE</td></tr>");
        for (i = 0; i < n; i++)
        {
            printf("<tr><td>%s</td><td>%s</td></tr>", p[i].key, p[i].value);
        }
